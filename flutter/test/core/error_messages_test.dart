...
void main() {
  group('apiErrorMessage', () {
    test(
      'network failure (offline) shows a friendly RU message, not the raw exception',
      () {
    ...
    test(
      'every ApiErrorType has both a RU and a KZ message and they differ',
      () {
        for (final type in ApiErrorType.values) {
          ...
          expect(
            ru,
            isNot(equals(kz)),
            reason: '${type.name} RU/KZ text should not be identical',
          );
        }
      },
    );
    ...
    test(
      'a non-ApiException error still gets a safe fallback message instead of crashing',
      () {
        final message = apiErrorMessage(
          AppLocale.ru,
          StateError('something internal broke'),
        );
