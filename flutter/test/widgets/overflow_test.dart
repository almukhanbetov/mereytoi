...
final _longListing = Listing(
  ...
  category: const Category(
    id: 1,
    slug: 'hosts',
    nameRu: 'Ведущие и очень длинное название категории',
    nameKz: 'Жүргізушілер',
    position: 0,
  ),
  nameRu:
      'Очень длинное название услуги, которое обязательно должно перенестись на несколько строк если не ограничено',
  nameKz: 'Ұзақ атау',
...
// Same worst case as `_longListing` plus a set `capacity` — Этап 10Б-Б1
// added a capacity readout (icon + number) next to the city on both
// restaurant card shapes, sharing the same Row as the city text, which is
// exactly the shape that has overflowed elsewhere in this app before.
final _longListingWithCapacity = Listing(
  id: 48,
  categoryId: 1,
  category: const Category(
    id: 1,
    slug: 'venues',
    nameRu: 'Рестораны',
    nameKz: 'Мейрамханалар',
    position: 0,
  ),
  nameRu:
      'Очень длинное название ресторана, которое обязательно должно перенестись если не ограничено',
  nameKz: 'Ұзақ атау',
  descriptionRu: 'Описание',
  descriptionKz: 'Сипаттама',
  city: 'Алматы (очень длинное название города для проверки)',
...
final _longCategory = const Category(
  ...
  slug: 'venues',
  nameRu:
      'Очень длинное название категории, которое может перенестись на несколько строк',
...
final _manyLongCategories = List.generate(
  ...
  (i) => Category(
    ...
    slug: 'cat-$i',
    nameRu:
        'Очень длинное название категории номер $i для проверки переполнения',
...
void main() {
  ...
  testWidgets(
    'ServiceCard with long name/city/huge price does not overflow at 2-column grid width',
    (tester) async {
      ...
      ); // let the image fade-in/skeleton settle
  ...
  testWidgets(
    'ServiceListTile (the primary services-list card) with long name/city does not overflow',
    (tester) async {
      await tester.pumpWidget(
        _wrap(
          SizedBox(
            width:
                320 -
                40, // full-width list row minus the screen's horizontal padding
  ...
  for (final width in [320.0, 360.0, 393.0, 430.0]) {
    testWidgets(
      'ServiceCard with long city + capacity does not overflow at ${width.toInt()}dp (real Home carousel width)',
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            SizedBox(
              width: 168, // the real fixed width Home's carousel gives it
              child: ServiceCard(
                listing: _longListingWithCapacity,
                locale: AppLocale.ru,
    ...
    testWidgets(
      'ServiceListTile with long city + capacity does not overflow at ${width.toInt()}dp',
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            SizedBox(
  ...
  testWidgets('ServiceCard shows the capacity readout when known', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(
  ...
  testWidgets(
    'ServiceCard omits the capacity readout when the listing has none (a non-venue service)',
    (tester) async {
      await tester.pumpWidget(
        _wrap(
          SizedBox(
            width: 168,
            child: ServiceCard(
              listing: _longListing, // capacity defaults to 0
              locale: AppLocale.ru,
  ...
  testWidgets('CategoryCard with a long two-line name does not overflow', (
    tester,
  ) async {
    await tester.pumpWidget(
      _wrap(
  ...
  testWidgets(
    'Cart screen empty state renders without overflow at 320px width',
    (tester) async {
  ...
  // being verified here.
  testWidgets(
    'CategoryFilterBar: one-time nudge plays and settles back to the start, lane holds every category with no overflow',
    (tester) async {
      await tester.pumpWidget(
        _wrap(
          SizedBox(
  ...
  testWidgets(
    'CategoryFilterBar "Все категории" chip opens the picker sheet and reports a pick',
    (tester) async {
  ...
  for (final size in const [
    ...
    testWidgets(
      'CategoryFilterBar lane + picker sheet grid render without overflow at ${size.width.toInt()}x${size.height.toInt()}',
      (tester) async {
        await tester.pumpWidget(
          _wrap(
            Padding(
